...
namespace WebGLSupport
{
    ...
    public class WebGLInput : MonoBehaviour, IComparable<WebGLInput>
    {
        ...
        private IInputField Setup()
        {
            ...
            if (displayObjInfo)
            {
                if (displayObjInfo.displayObject.gOwner is IInputFieldAdapter adapter)
                {
                    return adapter.CreateInputFieldAdapter();
                }
                else
                {
...
// #endif // TMP_WEBGL_SUPPORT
        ...
        private void Awake()
        {
            ...
            if (Application.isMobilePlatform)
            {
                ...
                {
                    var handler=gameObject.AddComponent<WebGLInputMobile>();
                    input.RegisterTouchBegin(handler);
        ...
        {
            var rect = GetScreenCoordinates(input);
            // モバイルの場合、強制表示する
        ...
        public void OnSelect()
        {
            ...
            Debug.Log($"WebGLInputCreate: {rect.x}, {rect.y},{rect.width},{rect.height},{fontSize},{input.text},{input.placeholder},isPassword:{isPassword},isHidden:{isHidden},isMobilePlatform:{Application.isMobilePlatform}");
            ...
            Debug.Log($"WebGLInputCreate: id:{id}");
            ...
            {
                Debug.Assert(input.text!=null,"input.text cannot be null");
                WebGLInputPlugin.WebGLInputSetSelectionRange(id, 0, input.text?.Length??0);
            }
            
            WebGLWindow.OnBlurEvent += OnWindowBlur;
            Debug.Log("OnSelect-done");
        }
        ...
        /// <returns></returns>
        Rect GetScreenCoordinates(IInputField input0)
        {
            var uiElement = input0.RectTransform();
            Canvas canvas;
            ...
            {
                var boundRect = input0.GetBounds();
                ref Vector3[] theCornersArray = ref worldCorners;
                theCornersArray[0] = new Vector3(boundRect.x, boundRect.y, 0.0f);
                theCornersArray[1] = new Vector3(boundRect.x, boundRect.yMax, 0.0f);
                theCornersArray[2] = new Vector3(boundRect.xMax, boundRect.yMax, 0.0f);
                theCornersArray[3] = new Vector3(boundRect.xMax, boundRect.y, 0.0f);
                canvas = null;
            }
            ...
            if (canvas != null)
            {
                useCamera = (canvas.renderMode != RenderMode.ScreenSpaceOverlay);
                if (useCamera)
                {
        ...
        static void OnBlur(int id)
        {
            ...
            Input.ResetInputAxes(); // Inputの状態リセット
#endif
            if (instances[id].isActiveAndEnabled)
            {
        ...
        void Update()
        {
            ...
            // Debug.Log($"IsFocuse: {input != null}, {input.isFocused}");
            // 未登録の場合、選択する
        ...
        {
            var a = GetScreenCoordinates(input);
            var b = GetScreenCoordinates(other.input);
